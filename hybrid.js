...
function createWorker(self) {
  ...
  function processPlyBuffer(inputBuffer) {
    ...
    var texwidth = 1024 * 4;
    var texheight = Math.ceil((4 * vertexCount) / texwidth);
...
void main () {
    ...
    vColor = 
      ...
      vec4((rgba) & 0xffu, (rgba >> 8) & 0xffu, (rgba >> 16) & 0xffu, (rgba >> 24) & 0xffu) / 255.0;
...
async function main() {
  ...
  worker.onmessage = (e) => {
    if (e.data.texdata) {
      ...
      const json = new TextEncoder().encode(
        JSON.stringify([{ type: "splat", size: texdata.byteLength, texwidth: texwidth, texheight: texheight, cameras: cameras }])
      );
      const magic = new Uint32Array(2);
      magic[0] = 0x674b;
      magic[1] = json.length;
      const blob = new Blob([magic.buffer, json.buffer, texdata.buffer], { type: "application/octet-stream" });
  ...
  function showCameraInfo(matrix, url) {
    ...
    if (!infoDiv) {
      ...
      infoDiv.style.cssText = `
        ...
        color: white; padding: 15px; border-radius: 5px; font-family: monospace;
        font-size: 12px; max-width: 500px; z-index: 1000; word-wrap: break-word;
      `;
      document.body.appendChild(infoDiv);
    }
    infoDiv.innerHTML = `
      <div style="margin-bottom: 10px; color: #4CAF50; font-weight: bold;">✓ Camera Position Copied!</div>
      ...
      <div style="margin-top: 10px; font-size: 11px; color: #888;">Press 'R' to hide | Press 'C' again to update</div>
    `;
    infoDiv.style.display = "block";
    setTimeout(() => { infoDiv.style.opacity = "0.5"; }, 5000);
  }
  ...
  const frame = (now) => {
    ...
    for (let gamepad of gamepads) {
      ...
      if (gamepad.buttons[4].pressed && !leftGamepadTrigger) { camera = cameras[(cameras.indexOf(camera) + 1) % cameras.length]; inv = invert4(getViewMatrix(camera)); carousel = false; }
      if (gamepad.buttons[5].pressed && !rightGamepadTrigger) { camera = cameras[(cameras.indexOf(camera) + cameras.length - 1) % cameras.length]; inv = invert4(getViewMatrix(camera)); carousel = false; }
  ...
  const url = params.get("url") ? new URL(params.get("url"), "https://huggingface.co/datasets/stpete2/splatv/resolve/main/") : "model.splatv";
